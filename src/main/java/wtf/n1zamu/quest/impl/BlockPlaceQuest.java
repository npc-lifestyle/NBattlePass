...
public class BlockPlaceQuest implements Quest<BlockPlaceEvent> {
    @Override
    public Runnable getExecutionTask(@NonNull BlockPlaceEvent event) {
        return new BukkitRunnable() {
            @Override
            public void run() {
                Player player = event.getPlayer();
                Block block = event.getBlock();
                if (!NBattlePass.getInstance().getQuestDataBase().hasQuest(player.getName(), QuestType.PLACE_BLOCKS)) {
                    return;
                }
                Map<Material, QuestObject> materialQuestMap = QuestUtil.getMaterialsByQuests(player, QuestType.PLACE_BLOCKS);
